  ...
  $($statsCalEvents.items) | Foreach-Object -ThrottleLimit 200 -Parallel {
    ...
    if (($PSItem.summary).contains("`u{1F95A}")) {
      switch ($($currDay).DayOfWeek) {
        "Monday" { ($USING:eggsPerDay)[0]++ }
        "Tuesday" { ($USING:EggsPerDay)[1]++ }
        "Wednesday" { ($USING:EggsPerDay)[2]++ }
        "Thursday" { ($USING:EggsPerDay)[3]++ }
        "Friday" { ($USING:EggsPerDay)[4]++ }
        "Saturday" { ($USING:EggsPerDay)[5]++ }
        "Sunday" { ($USING:EggsPerDay)[6]++ }
...
if ($result -eq [System.Windows.Forms.DialogResult]::OK) {
  $x = $textBox.Text
  Add-NewEggs -newEggs $x -eggCount $eggCount
}